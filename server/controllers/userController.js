...
const postRegistration = async(req, res, next) => {
    try {
        if (!res.body.email || req.body.email.length === 0) return next(new ApiError('Invalid email for user', 400))
            if (!res.body.password || req.body.password.length < 8) return next(new ApiError('Invalid password for user', 400))
...
const createUserObject = (id, email, token = undefied) => {
    return {
        'id': id,
        'email': email,
        ...(token !== undefied) && {'token': token}
    }
}
